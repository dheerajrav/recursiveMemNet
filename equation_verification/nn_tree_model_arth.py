...
from equation_verification.dataset_loading_arth import BinaryEqnTree
...
def build_nnTree(model_class,
    ...
    if model_class == "NNTrees":
        ...
        model = LSTMTrees(num_hidden,
                             ...
                             share_memory_params,
                             dropout)
    elif model_class == "GRUTreesV1":
        model = GRUTreesV1(num_hidden,
                         num_embed,
                         memory_size,
                         share_memory_params,
                         dropout)
    elif model_class == "GRUTreesV2":
        model = GRUTreesV2(num_hidden,
                           num_embed,
                           memory_size,
                           share_memory_params,
                           dropout)
    elif model_class == "GRUTreesV3":
        model = GRUTreesV3(num_hidden,
                           num_embed,
                           memory_size,
                           share_memory_params,
                           dropout)
    elif model_class == "GRUTreesV4":
        model = GRUTreesV4(num_hidden,
                           num_embed,
                           memory_size,
                           share_memory_params,
                           dropout)
    elif model_class == "GRUTreesV5":
        model = GRUTreesV5(num_hidden,
                           num_embed,
                           memory_size,
                           share_memory_params,
                           dropout)
    elif model_class == "GRUTreesV6":
        model = GRUTreesV6(num_hidden,
                           num_embed,
                           memory_size,
                           share_memory_params,
                           dropout)
...
class GRUTreesV1(torch.nn.Module):
    def __init__(self, num_hidden, num_embed, memory_size, share_memory_params,
                 dropout):
        super().__init__()

        # For every name in UNARY FUNCTIONS, it is creating a Unary
        for name in UNARY_FNS:
            setattr(self, name, UnaryGRUNode(num_hidden, num_hidden))

        for name in BINARY_FNS:
            setattr(self, name, BinaryGRUNodeV1(num_input=num_hidden,
                                               num_hidden=num_hidden))
    ...
    def forward(self, tree, trace=None):
        ...
        if tree.is_binary:
            hl = self(tree.lchild, trace=trace.lchild if trace else None)
            hr = self(tree.rchild, trace=trace.rchild if trace else None)
            nn_block = getattr(self, tree.function_name)
            return nn_block((hl), (hr), trace=trace)
        elif tree.is_unary:
            hl = self(tree.lchild, trace=trace.lchild if trace else None)
            nn_block = getattr(self, tree.function_name)
            if tree.function_name in {NUMBER_DECODER, NUMBER_ENCODER,
...
class GRUTreesV2(torch.nn.Module):
    def __init__(self, num_hidden, num_embed, memory_size, share_memory_params,
                 dropout):
        super().__init__()
        ...
        for name in BINARY_FNS:
            setattr(self, name, BinaryGRUNodeV2(num_input=num_hidden,
                                               num_hidden=num_hidden))
...
class GRUTreesV3(torch.nn.Module):
    def __init__(self, num_hidden, num_embed, memory_size, share_memory_params,
                 dropout):
        super().__init__()
        ...
        for name in BINARY_FNS:
            setattr(self, name, BinaryGRUNodeV3(num_input=num_hidden,
                                               num_hidden=num_hidden))
...
class GRUTreesV4(torch.nn.Module):
    def __init__(self, num_hidden, num_embed, memory_size, share_memory_params,
                 dropout):
        super().__init__()
        ...
        for name in BINARY_FNS:
            setattr(self, name, BinaryGRUNodeV4(num_input=num_hidden,
                                               num_hidden=num_hidden))
...
class GRUTreesV5(torch.nn.Module):
    def __init__(self, num_hidden, num_embed, memory_size, share_memory_params,
                 dropout):
        super().__init__()
        ...
        for name in BINARY_FNS:
            setattr(self, name, BinaryGRUNodeV5(num_input=num_hidden,
                                               num_hidden=num_hidden))
...
class GRUTreesV6(torch.nn.Module):
    def __init__(self, num_hidden, num_embed, memory_size, share_memory_params,
                 dropout):
        super().__init__()
        ...
        for name in BINARY_FNS:
            setattr(self, name, BinaryGRUNodeV6(num_input=num_hidden,
                                               num_hidden=num_hidden))
...
class UnaryGRUNode(torch.nn.Module):
    def __init__(self, num_input, num_hidden):
        super().__init__()
        self.reduce_gate = nn.Linear(num_input, num_hidden, bias=True)
        self.update_gate = nn.Linear(num_input, num_hidden, bias=True)
        self.hidden_gate = nn.Linear(num_input, num_hidden, bias=True)

    def forward(self, inp, trace=None):
        """

        Args:
            inp: ((num_input,), (num_hidden))

        """
        h_tminus_1 = inp # GRUs only return h as output, and no context

        r_t = F.sigmoid(self.reduce_gate(h_tminus_1))
        z_t = F.sigmoid(self.update_gate(h_tminus_1))
        h_tcap = F.tanh(self.hidden_gate(r_t * h_tminus_1))
        h_t = (1 - z_t) * h_tcap + z_t * h_tminus_1

        if trace:
            trace.output = h_t.tolist()

        return h_t

class BinaryGRUNodeV1(torch.nn.Module):
    def __init__(self, num_input, num_hidden):
        super().__init__()
        self.reduce_left = nn.Linear(num_input, num_hidden, bias=False)
        self.reduce_right = nn.Linear(num_input, num_hidden, bias=False)
        self.reduce_bias = nn.Parameter(torch.FloatTensor([0] * num_hidden))
        self.update_left = nn.Linear(num_input, num_hidden, bias=False)
        self.update_right = nn.Linear(num_input, num_hidden, bias=False)
        self.update_bias = nn.Parameter(torch.FloatTensor([0] * num_hidden))
        self.hidden = nn.Linear(num_input, num_hidden, bias=False)
        #TODO: different hiddens (left and right)
        self.hidden_bias = nn.Parameter(torch.FloatTensor([0] * num_hidden))
    ...
    def forward(self, input_left, input_right, trace=None):
        ...
        Args:
        """
        hl_tminus_1 = input_left
        hr_tminus_1 = input_right

        r_t = F.sigmoid(self.reduce_left(hl_tminus_1) +
                        self.reduce_right(hr_tminus_1) +
                        self.reduce_bias)

        z_t = F.sigmoid(self.update_left(hl_tminus_1) +
                        self.update_right(hr_tminus_1) +
                        self.update_bias)

        h_tcap = F.tanh(self.hidden(r_t * hl_tminus_1 + r_t * hr_tminus_1) + self.hidden_bias)
        h_t = z_t * (hl_tminus_1 + hr_tminus_1) + (1 -z_t) * h_tcap
...
class BinaryGRUNodeV2(torch.nn.Module):
    def __init__(self, num_input, num_hidden):
        super().__init__()
        self.reduce_left = nn.Linear(num_input, num_hidden, bias=False)
        self.reduce_right = nn.Linear(num_input, num_hidden, bias=False)
        self.reduce_left_bias = nn.Parameter(torch.FloatTensor([0] * num_hidden))
        self.reduce_right_bias = nn.Parameter(torch.FloatTensor([0] * num_hidden))
        self.update_left = nn.Linear(num_input, num_hidden, bias=False)
        self.update_right = nn.Linear(num_input, num_hidden, bias=False)
        self.update_bias = nn.Parameter(torch.FloatTensor([0] * num_hidden))
        self.hidden = nn.Linear(num_input, num_hidden, bias=False)
    ...
    def forward(self, input_left, input_right, trace=None):
        ...
        r_lt = F.sigmoid(self.reduce_left(hl_tminus_1) +
                        self.reduce_left_bias)

        r_rt = F.sigmoid(self.reduce_right(hr_tminus_1) +
                        self.reduce_right_bias)

        z_t = F.sigmoid(self.update_left(hl_tminus_1) +
                        self.update_right(hr_tminus_1) +
                        self.update_bias)

        h_tcap = F.tanh(self.hidden(r_lt * hl_tminus_1 + r_rt * hr_tminus_1) + self.hidden_bias)
        #h_t = z_lt * hl_tminus_1 + z_rt * hr_tminus_1 + (1 - z_rt) * h_tcap + (1 - z_lt) * h_tcap
        h_t = z_t * (hl_tminus_1 + hr_tminus_1) + (1 -z_t) * h_tcap
...
class BinaryGRUNodeV3(torch.nn.Module):
    def __init__(self, num_input, num_hidden):
        super().__init__()
        self.reduce_left = nn.Linear(num_input, num_hidden, bias=False)
        self.reduce_right = nn.Linear(num_input, num_hidden, bias=False)
    ...
    def forward(self, input_left, input_right, trace=None):
        ...
        h_tcap = F.tanh(self.hidden(r_t * hl_tminus_1 + r_t * hr_tminus_1) + self.hidden_bias)
        h_t = (1 - z_t) * (hl_tminus_1 + hr_tminus_1) * 0.5 + z_t * h_tcap
...
class BinaryGRUNodeV4(torch.nn.Module):
    def __init__(self, num_input, num_hidden):
        super().__init__()
        self.reduce_left = nn.Linear(num_input, num_hidden, bias=False)
        self.reduce_right = nn.Linear(num_input, num_hidden, bias=False)
...
class BinaryGRUNodeV5(torch.nn.Module):
    def __init__(self, num_input, num_hidden):
        super().__init__()
        self.reduce_left = nn.Linear(num_input, num_hidden, bias=False)
        self.reduce_right = nn.Linear(num_input, num_hidden, bias=False)
        ...
        self.update_left = nn.Linear(num_input, num_hidden, bias=False)
        self.update_right = nn.Linear(num_input, num_hidden, bias=False)
        self.update_left_bias = nn.Parameter(torch.FloatTensor([0] * num_hidden))
        self.update_right_bias = nn.Parameter(torch.FloatTensor([0] * num_hidden))
        self.hidden = nn.Linear(num_input, num_hidden, bias=False)
        self.hidden_bias = nn.Parameter(torch.FloatTensor([0] * num_hidden))
    ...
    def forward(self, input_left, input_right, trace=None):
        ...
        z_lt = F.sigmoid(self.update_left(hl_tminus_1) +
                         self.update_left_bias)

        z_rt = F.sigmoid(self.update_right(hr_tminus_1) +
                         self.update_right_bias)

        h_tcap = F.tanh(self.hidden(r_lt * hl_tminus_1 + r_rt * hr_tminus_1) + self.hidden_bias)
        h_t = ((1 - z_lt) * hl_tminus_1 + (1 - z_rt) * hr_tminus_1 + (z_lt + z_rt) * h_tcap) * 0.5
...
class BinaryGRUNodeV6(torch.nn.Module):
    def __init__(self, num_input, num_hidden):
        super().__init__()
        self.reduce_left = nn.Linear(num_input, num_hidden, bias=False)
        self.reduce_right = nn.Linear(num_input, num_hidden, bias=False)
        ...
        self.update_right_bias = nn.Parameter(torch.FloatTensor([0] * num_hidden))
        self.hidden = nn.Linear(num_input, num_hidden, bias=False)
        self.hidden_left_bias = nn.Parameter(torch.FloatTensor([0] * num_hidden))
        self.hidden_right_bias = nn.Parameter(torch.FloatTensor([0] * num_hidden))
    ...
    def forward(self, input_left, input_right, trace=None):
        ...
        h_ltcap = F.tanh(self.hidden(r_lt * hl_tminus_1) + self.hidden_left_bias)
        h_rtcap = F.tanh(self.hidden(r_rt * hr_tminus_1) + self.hidden_right_bias)
        h_t = ((1 - z_lt) * hl_tminus_1 + (1 - z_rt) * hr_tminus_1 + z_lt * h_ltcap + z_rt * h_rtcap) * 0.5
