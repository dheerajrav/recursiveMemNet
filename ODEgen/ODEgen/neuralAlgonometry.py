...
# fVocab = copy.deepcopy(functionVocab)
...
# 	ctr+=1
# ################################################################################
# #Pass in trainEquations[0] of class EquationTree
# #def constructFixedParamNames(eq, name): 
# #	eq
...
# 		for key, cellP in zip(argDict.keys(), cell_list):
#                         # print "key: ", key, "argDict: ", argDict[key] 
#                         if isinstance(argDict[key],list):
#                                 cellP.value = str(argDict[key]) 
#                         else: 
...
def writeJson(path, equations, ranges, variables, labels, maxDepth):
	data = [[] for _ in range(maxDepth)]
	# print('data len in write:', len(data))
	# print('data:',data)
...
def readBlockJsonEquations(path, trainDepth=None, testDepth=None, excludeFunc='', splitRatio=0.8, devSplit=0.9):
	random.seed(1)
	equations = []
	variables = []
	ranges = []
	...
	# for daidguh in data:
	# 	print(len(daidguh))
	# 	dataln = dataln + len(daidguh)
	# print('dataln:',dataln)
	# print('orig data len full:', len(data))

	if trainDepth!=None:
		ind = [indInside-1 for indInside in trainDepth]
		# print('ind:',ind)
		# ind = [0]
		# ind.extend(trainDepth)
		teIn = [indInside-1 for indInside in testDepth]
		# print('teIn:',teIn)
		# teIn = [0]
	...
	indices = list(range(0,len(equations)))
	...
	if testData==[]:
		...
		print "test data is non empty"
		trInd = indices
		teInd = range(0,len(teEquations))
	...
	indices = list(range(0,len(trainEquations)))
	random.shuffle(indices)
	# iind = [0]
	# iind.extend(indices)
	# indices = iind
	...
	# print('total data len:', len(trEquations), len(devEquations), len(testEquations))
...
# 		return True
# 	# #new
# 	# elif sympify(sympyEq) == False:
# 	# 	return False
# 	# else:
# 	# 	return sympify(sympyEq)
# 	# # end new
# 	else:
# 		return False
...
# 		# print 'eq:', equation
# 		# equation.preOrder()
# 		nodeNumbers = [arg.number for arg in equation.args if arg.func!='NumberDecoder']
# 		# print nodeNumbers
# 		assert len(nodeNumbers) == 1, 'only one node number should be returned, but returned {0}'.format(len(nodeNumbers))
# 	else:
...
# 					return newArg
# 				else:
# 					newArg = returnNode(arg, nodeNum)
# 					if newArg != None:
# 						return newArg
# 		return newArg

# def genMSEeq(equation, label, variable, domainNumbers):
# 	assert equation.isNumeric(), 'equation should be numeric'
# 	# print 'variable:', variable
# 	assert variable == {}, 'numeric data should not have variables'
# 	assert label.asnumpy() == 1, 'only correct equations should be considered for regression'
# 	resEqs = []
# 	resLabels = []
# 	resVars = []

# 	eqNums = equation.extractNums()
# 	# indicate if the number is negative or not:
# 	# print 'equation before changing', equation.prettyOld()
# 	flag = 0
# 	numDecInd = -1
# 	# print 'len args:', len(equation.args)
# 	for ind, arg in enumerate(equation.args):
# 		if arg.func == 'Number':
# 			assert flag!=-1, 'cannot have both negative and positive results' 
# 			flag = 1
# 			arg.func = 'MSEnumber'
# 			newLabel = float(arg.varname) # round(float(arg.varname),2)
# 			# print 'newLabel:', newLabel
# 			if newLabel not in eqNums:
# 				eqNums.append(newLabel)
# 		elif arg.func == 'Mul' and (   (arg.args[0].func=='NegativeOne' and arg.args[1].func=='Number') 
# 		                            or (arg.args[1].func=='NegativeOne' and arg.args[0].func=='Number') ):
# 			assert flag!=1, 'cannot have both negative and positive results' 
# 			flag = -1
# 			if arg.args[0].func!='NegativeOne':
# 				newLabel = -float(arg.args[0].varname) # -round(float(arg.args[0].varname),2)
# 			elif arg.args[1].func!='NegativeOne':
# 				newLabel = -float(arg.args[1].varname) # -round(float(arg.args[1].varname),2)
# 			else:
# 				raise AssertionError, 'this should not have happened'
# 			equation.args[ind] = EquationTree(func='MSEnumber', args=[], varname=str(newLabel))
# 			if newLabel not in eqNums:
# 				eqNums.append(newLabel)
# 			# print 'newLabel:', newLabel
# 		else:
# 			# print "function:", arg.func
# 			numDec = EquationTree(func='NumberDecoder', args=[arg], varname='')
# 			equation.args[ind] = numDec
# 			numDecInd = ind
# 	#removing equality altogether
# 	# equation = equation.args[numDecInd]
# 	equation.enumerize_queue()
# 	# print 'equation after changing:', equation.prettyOld()
# 	# raw_input('press enter for the next eq')
# 	# equation.preOrder()

# 	resEqs.append(equation)
# 	resLabels.append(mx.nd.array([round(newLabel,2)], dtype='float32'))
# 	resVars.append(variable)

# 	negNums = []
# 	eqNums.append(newLabel)
# 	for n in eqNums:
# 		negNums.append(-n)
# 	eqNums.extend(negNums)

# 	for n in eqNums:
# 		if n not in domainNumbers:
# 			domainNumbers.append(n)
# 			leaf = EquationTree(func='Number', args=[], varname=str(n))
# 			root = EquationTree(func='NumberDecoder', args=[leaf], varname='')
# 			root = EquationTree(func='Equality', 
# 					            args=[root, EquationTree(func='MSEnumber', args=[], varname=str(n))], 
# 					            varname='')
# 			root.enumerize_queue()
# 			# print 'root1:', root.prettyOld()
# 			rootVars = root.extractVars()

# 			resEqs.append(root)
# 			resLabels.append(mx.nd.array([round(n,2)], dtype='float32'))
# 			resVars.append(rootVars)

# 			if n < 0 : 
# 				negOne = EquationTree(func='NegativeOne', args=[], varname=str(-1))
# 				leaf = EquationTree(func='Number', args=[], varname=str(abs(n)))
# 				negOneFlag = random.choice([0,1])
# 				if negOneFlag ==0:
# 					root = EquationTree(func='Mul', args=[negOne, leaf], varname='')
# 				else:
# 					root = EquationTree(func='Mul', args=[leaf, negOne], varname='')
# 				root = EquationTree(func='NumberDecoder', args=[root], varname='')
# 				root = EquationTree(func='Equality', 
# 					                args=[root, EquationTree(func='MSEnumber', args=[], varname=str(n))], 
# 					                varname='')
# 				# print 'root2:', root.prettyOld()
# 				root.enumerize_queue()
# 				rootVars = root.extractVars()
...
# 				negOneFlag = random.choice([0,1])
# 				if negOneFlag == 0:
# 					root = EquationTree(func='Mul', args=[root, copy.deepcopy(negOne)], varname='')
# 				else:
# 					root = EquationTree(func='Mul', args=[copy.deepcopy(negOne), root], varname='')
# 				root = EquationTree(func='NumberDecoder', args=[root], varname='')
# 				root = EquationTree(func='Equality', 
# 					                args=[root, EquationTree(func='MSEnumber', args=[], varname=str(n))], 
# 					                varname='')
...
# 	return [resEqs, resLabels, resVars]


# ################################################################################
# # NN functions: # 
# def buildNNTree(treeType, parsedEquation, num_hidden, params, emb_dimension, varDict={}, dropout=0.0, **kwargs):
# 	# TODO: handle range
...
# 			            emb_dimension=1, nodeNumber=parsedEquation.number, dropout=dropout, **kwargs)
...
# 	# added for MSE training
# 	elif func == 'MSEnumber':
# 		assert parsedEquation.varname != '', ('leaf {0} should have input').format(str(parsedEquation))
# 		root = treeType(prefix='MSEnumber', num_hidden=num_hidden,
# 		                params=params, inputName=parsedEquation.varname, args=[], 
# 		                emb_dimension=1, nodeNumber=parsedEquation.number, dropout=dropout, **kwargs)
# 	# end
...
# 			                       emb_dimension=emb_dimension, **kwargs))
...
# 				# was outside the if before adding MSE:
# 				code.append(copy.deepcopy(vocab[key]))
# 			# added for MSE training
# 			elif eqStr == 'MSEnumber':
# 				pass
# 			# end
# 			else:
# 				key = eqStr
...
class EquationTree(object):
	...
	def returnFunctionNames(self, names=[]):
		
		# COMMENT: THIS FUNCTION DOES NOT RETURN THE LEAF NODE NAMES
		if self.args != []:
			names.append(self.func)

		if len(self.args) > 2:
			raise AssertionError('not a binary tree! check buildEq')
		
		for arg in self.args:
			names = arg.returnFunctionNames(names)

		return names

	def enumerize_queue(self, currentCtr=0, verbose=0):
		...
		while len(depthQueue[0])!=0 or len(depthQueue[1])!=0:
			# print 'depthQueue0:', depthQueue[0]
			# print 'depthQueue1:', depthQueue[1]
			while len(depthQueue[0]) != 0:
				# print 'depthQueue0 inner while:', depthQueue[0]
				if verbose:
					print('first while inner')
					self.preOrder()
					print('1: --------------------------------')
			...
			flag1 = 0
			if verbose:
				print('after first while inner')
				self.preOrder()
				print('1 after: --------------------------------')
			while len(depthQueue[1]) != 0:
				# print 'depthQueue1 inner while:', depthQueue[1]
				if verbose:
					print('second while inner')
					self.preOrder()
					print('2: --------------------------------')
			...
			if verbose:
				print('after second while inner')
				self.preOrder()
				print('2 after: --------------------------------')
	...
	def isUnique(self, numList=[]):
		...
		if sorted(numList) == list(set(numList)):
			...
			# print 'set numList:', list(set(numList))
			return True
		else:
			# print 'numList:', sorted(numList)
			# print 'set numList:', list(set(numList))
