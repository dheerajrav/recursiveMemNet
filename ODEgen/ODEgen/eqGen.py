...
sys.path.append('/home/billyjayan/workspace/cub/csci_5622/latex2sympy')
import json
from sympy import *
import pprint
...
functionTwoInp = ['Equality', 'Add', 'Mul', 'Pow', 'log', 'Derivative']#'Min', 'Max','atan2', 'Div'
...
consts = ['NegativeOne', 'Pi', 'One',
          'Half', 'Integer', 'Rational', 'Float', 'ComplexInfinity']#, 'Float', 'NaN', 'Infinity', 'Exp1',
...
def encodePositiveNumber(num):
	strNum = str(num)
	# tree = Mul(Pow(10, len(strNum - 1)), strNum[0])
	exponent = EquationTree(func = 'Integer', args = [], varname = str(len(strNum) - 1))
	base = EquationTree(func = 'Integer', args = [], varname = "10")
	power = EquationTree(func = 'Pow', args = [base, exponent])
	coeff = EquationTree(func = 'Integer', args = [], varname = strNum[len(strNum) - 1])
	tree = EquationTree(func = 'Mul', args = [coeff, power])
	for i in range(1, len(strNum)):
		exponent = EquationTree(func = 'Integer', args = [], varname = str(len(strNum) - i - 1))
		base = EquationTree(func = 'Integer', args = [], varname = "10")
		power = EquationTree(func = 'Pow', args = [base, exponent])
		coeff = EquationTree(func = 'Integer', args = [], varname = strNum[i])
		child = EquationTree(func = 'Mul', args = [coeff, power])
		tree = EquationTree(func = 'Add', args = [tree, child])
	return tree

def encodeNumber(num):
	if num >= 0:
		return encodePositiveNumber(num)
	else:
		positive = encodePositiveNumber(-num)
		negative = EquationTree(func = 'NegativeOne', args = [], varname = "-1")
		return EquationTree(func = 'Mul', args = [negative, positive])

def encodeFloat(float_num):
	mantissa = float_num[0]
	e = float_num[1]
	mantissa_tree = encodeNumber(mantissa)
	for i in range(0, len(e)):
		exponent = EquationTree(func = 'Integer', args = [], varname = str(-i - 1))
		base = EquationTree(func = 'Integer', args = [], varname = "10")
		power = EquationTree(func = 'Pow', args = [base, exponent])
		coeff = EquationTree(func = 'Integer', args = [], varname = e[i])
		child = EquationTree(func = 'Mul', args = [coeff, power])
		tree = EquationTree(func = 'Add', args = [mantissa_tree, child])
	mantissa_tree = EquationTree(func = 'Add', args = [tree, mantissa_tree])
	return mantissa_tree

# func='', args=[], varname='', number=-1, depth=-1)
def buildEq(parsedEquation, variables):
	...
	while func[-1]=='\'' or func[-1]=='>':
		func = func[:-1]
	if func == 'log':
		print 'Dilly ' + str(parsedEquation.args) + ' ' #+ str(len(parsedEquation.func.args))
	...
	if len(parsedEquation.args)==0:
		...
		# print key
		if func == 'Integer':
			number = encodeNumber(parsedEquation.p)
			return number

		if func == 'Float':
			number = encodeFloat(str(round(parsedEquation ,2)).split('.'))
			return number

		if func == 'Rational' and (parsedEquation.p != 2 or parsedEquation.q != 5):
			# denominator = EquationTree(func = 'Integer', args = [], varname = str(parsedEquation.q))
			denominator = encodeNumber(parsedEquation.q)
			# numerator = EquationTree(func = 'Integer', args = [], varname = str(parsedEquation.p))
			numerator = encodeNumber(parsedEquation.p)
			negative = EquationTree(func = 'NegativeOne', args = [], varname = "-1")
			power = EquationTree(func = 'Pow', args = [denominator, negative])
			mul = EquationTree(func = 'Mul', args = [numerator, power])
			return mul
...
def genNegEquation(eqTree, eqTreeVariables):
	...
	if node == None:
		...
		raise AssertionError, "node should not be none. There is a problem with numbering"
	badl = []
	for i in range(1,eq.numNodes):
		node1 = dummy.findNode(i)
		if(node1.func == 'Tuple'):
			leftc = node1.args[0]
			rightc = node1.args[1]
			badl.append(leftc.number)
			badl.append(rightc.number)
	while(node.func == 'Tuple' or node.number in badl or node.func == 'Derivative') :
		nodeNum = random.randrange(1, eq.numNodes)
		node = dummy.findNode(nodeNum)
	# if nodeNum == 0:
...
class EquationTree(object):
	...
	def growBothSides(self, eqVars):
		...
		if randFunc in functionTwoInpSet:
			...
			if orderFlag == 0:
				...
				selfCopy.args.append(EquationTree(func=randFunc, args=[self.args[1], inpCopy]))
			else:
				selfCopy.args.append(EquationTree(func=randFunc, args=[inp, self.args[0]]))
				selfCopy.args.append(EquationTree(func=randFunc, args=[inpCopy, self.args[1]]))
	...
	def extractVars(self):
		...
		def extractVarsRecurse(root):
			if len(root.args) == 0 and root.func in variablesSet and root.func != 'ComplexInfinity':
	...
	def isGrammarCompatible(self):
		# Leaf node.
		if len(self.args) == 0:
			if self.func == 'Integer':
				if int(self.varname) in intList:
					#print 'in int list'
					return True
				else:
					#print 'not in int list' + self.varname
					return False
			elif self.func == 'Rational':
				if self.varname in ratList:
					#print 'in rat list'
					return True
				else:
					#print 'not in rat list' + self.varname
					return False
			elif self.func == 'Float':
				#print 'float'
				return True
			elif self.func == 'Symbol':
				#print 'symbol'
				return True
			elif self.func in constsSet:
				#print 'consts ' + self.func
				return True
			else:
				#print 'not int or float or rat' + self.varname
				return False
		else:
			if self.func in functionOneInpSet or self.func in functionTwoInpSet or self.func == 'Tuple':
				for arg in self.args:
					if arg.isGrammarCompatible() == False:
						return False
				return True
			else:
				print 'func not in any set' + self.func
				return False
